...
import { ContentInvalidError, MultipleSubscriptionError, TopicNotFoundError, UserHasPermissionsError, UserNotFound } from "../shared/errors.ts";
import { Type } from "https://esm.sh/v135/ajv@8.12.0/dist/compile/util.js";
import { AIntertalk } from "../types/storage.ts";

const blankStorage = {
  async getTopic (_: string) { return null; },
  async getUser (_: string) { return null; },
  async getRole (_: string) { return null; },
  async getTopicStats (_: string) { return null; },
  async *getSubscriptions(): AsyncGenerator<any> { yield null; },
  async getSubscriptionState(_ : string) { return null; },
  async addContent<T>(_: string|undefined, __: string, ___: T[]) { return {lastId: 0}; },
  async addSubscription(_ : string, __ : string, ___ : string, ____ : number) { return {existed: false} },
  async validateContent<T> (_: string, __: T[]) {  },
}

function StubIntertalkClient<T>(content: T) {
  class Anon extends AIntertalk {
    constructor() { super(); }
...
Deno.test("Subscriptions.sync() | fails when topic is not present", async () => {
  ...
  await assertRejects(async () => {
    await sub.sync("http://example.com/", "subscription.example", "service-account", 60);
  }, TopicNotFoundError, "Topic \"subscription.example\" does not exist");
...
Deno.test("Subscriptions.sync() | fails when user is not present", async () => {
  ...
  await assertRejects(async () => {
    await sub.sync("http://example.com/", "subscription.example", "service-account", 60);
  }, UserNotFound, "User \"service-account\" does not exist");
...
Deno.test("Subscriptions.sync() | fails when user has permissions", async () => {
  ...
  await assertRejects(async () => {
    await sub.sync("http://example.com/", "subscription.example", "service-account", 60);
  }, UserHasPermissionsError, "User \"service-account\" does not have the PERMISSIONLESS role, so cannot be used as a service-account for retrieving subscriptions from another server");
});
...
Deno.test("Subscriptions.sync() | fails when subscription already present", async () => {
  ...
  await assertRejects(async () => {
    await sub.sync("http://example.com/", "subscription.example", "service-account", 60);
  }, MultipleSubscriptionError, "Another subscription already syncs to subscription.example");
});
...
Deno.test("Subscriptions.sync() | handles invalid content responses", async () => {
  ...
  await assertRejects(async () => {
    await sub.sync("http://example.com/", "subscription.example", "service-account", 60);
  }, ContentInvalidError, "The requested server returned a response to /content/<your-topic> without a \"content\" property");
});
