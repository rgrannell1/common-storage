...
type GetEventsRequest = z.infer<typeof GetEventsPathSchema> &
  z.infer<typeof GetEventsQuerySchema> & {
    stream: boolean;
    signal: AbortSignal;
  };
...
): Result<GetEventsResponse, RouteError> {
...
async function paginatedEventsResponse(
  deps: GetEventsDeps,
  params: GetEventsRequest,
): Promise<Result<GetEventsResponse, RouteError>> {
...
function eventsResponseParser(value: unknown): Result<RouteSuccess, RouteError> {
  const isStreamResponse = value !== null && typeof value === "object" &&
    "kind" in value && (value as GetEventsStream).kind === "stream";
  if (isStreamResponse) {
...
export function getEventsRoute(
  deps: GetEventsDeps,
): Route<null, GetEventsRequest, GetEventsResponse, RouteSuccess, RouteError> {
  return {
