...
type SuccessHandler<Kind extends RouteSuccess["kind"]> = (
  ctx: Context,
  success: Extract<RouteSuccess, { kind: Kind }>,
) => Response;

// Maps a discriminated error kind to a function that serialises it into an HTTP Response.
type ErrorHandler<Kind extends RouteError["kind"]> = (
  ctx: Context,
  error: Extract<RouteError, { kind: Kind }>,
) => Response;
...
const SUCCESS_HANDLERS: { [Kind in RouteSuccess["kind"]]: SuccessHandler<Kind> } = {
  ...
  stream:     (_ctx, success) => new Response(success.stream as ReadableStream, {
    status: STATUS_OK,
...
const ERROR_HANDLERS: { [Kind in RouteError["kind"]]: ErrorHandler<Kind> } = {
  parse_request: (ctx, error) =>
    ctx.json({ error: error.message, field: error.field }, STATUS_BAD_REQUEST),
  validation_error: (ctx, error) =>
    ctx.json({ error: error.message }, STATUS_UNPROCESSABLE),
  not_found: (ctx, error) =>
    ctx.json({ error: `Not found: ${error.resource}` }, STATUS_NOT_FOUND),
  internal: (ctx, _error) =>
    ctx.json({ error: "Internal server error" }, STATUS_INTERNAL_ERROR),
};
