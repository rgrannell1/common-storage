...
import { DEFAULT_FETCH_PAGE_SIZE, MERKLE_TREE_END, MERKLE_LEAF_SIZE } from "../../commons/constants.ts";
...
export async function syncOnce(config: SubscriptionConfig, storage: SyncStorage, logger: ILogger): Promise<void> {
  ...
  await applyDiffAndTail(storage, config, token, leafRanges, initialMaxId, logger);
  logger.info("subscription sync complete", undefined, { source: config.source, topic: config.topic });
}

// Fetches differing leaf ranges then tails the stream to catch writes that arrived during the diff rounds.
async function applyDiffAndTail(
  ...
  for (const range of leafRanges) {
    ...
    const lastId = await fetchAndReplicate(storage, config.topic, config.source, token, range.start + 1, MERKLE_LEAF_SIZE, logger);
